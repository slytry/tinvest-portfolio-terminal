import type { MoneyValue as _tinkoff_public_invest_api_contract_v1_MoneyValue, MoneyValue__Output as _tinkoff_public_invest_api_contract_v1_MoneyValue__Output } from '../../../../../../tinkoff/public/invest/api/contract/v1/MoneyValue';
export interface SandboxPayInResponse {
    'balance'?: (_tinkoff_public_invest_api_contract_v1_MoneyValue | null);
}
export interface SandboxPayInResponse__Output {
    'balance': (_tinkoff_public_invest_api_contract_v1_MoneyValue__Output | null);
}
